...
class MyBot(BaseBot):
    ...
    def _roll_bank_or_quit(self):
        ...
        if self.dice_remaining >= 5:
            return "r"
        if self.unbanked_points >= 1050 and self.dice_remaining < 5:
            return "b"
        if self.unbanked_points >= 450 and self.dice_remaining < 4:
            return "b"
        if self.unbanked_points >= 250 or self.dice_remaining < 3:
            return "b"
        if self.unbanked_points > 251:
